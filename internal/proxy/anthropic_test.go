...
func TestCheckPlanModel(t *testing.T) {
	for _, m := range []string{"claude-sonnet-5", "claude-opus-5", "sonnet", "gpt-5.4", "google/gemini-3.5-flash"} {
...
func TestHandleMessages_NonStreamE2E(t *testing.T) {
	...
	if resp.Model != "deepseek/deepseek-v4-flash" {
		t.Fatalf("model = %q, want mapped deepseek/deepseek-v4-flash", resp.Model)
	}
}
