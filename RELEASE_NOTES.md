...
- **DSH（DeepSeek Harness）兼容修复**：DSH 以  发送系统提示，代理此前未识别该角色导致
  带工具调用的多轮对话被上游 400 拒绝（ 校验失败）；现与  同等提取，
  未知角色兜底为 ， 各档位（off/low/medium/high/max）实测全部可用
- 新增  启动参数：打印请求/响应体到 headless-error.log，便于排查协议问题
