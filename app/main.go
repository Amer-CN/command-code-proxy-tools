...
var (
	...
	// 插件子模式（--plugin-tuanjie / --plugin-codebuddy / --plugin-notion / --plugin-lingxi / --plugin-zcoderemote / --desensitize）
	// 定义在 plugin_modes.go。
)
...
func main() {
	...
	// 插件子模式（团结 / CodeBuddy / Notion / 灵犀 / ZCode多开）：实现见 plugin_modes.go
	if *flagPluginTuanjie || *flagPluginCodebuddy || *flagPluginNotion || *flagPluginLingxi || *flagPluginZcodeRemote {
