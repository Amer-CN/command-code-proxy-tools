...
```
CommandCodeProxyDeck.exe        ← 唯一主程序，双击即用
...
├─ internal/
│   ├─ proxy + server + api     主代理核心（上游 dev2k6/command-code-proxy-server）
│   ├─ tuanjie/                 团结插件后端（8788）
...
| taskkill CommandCodeProxyDeck 进程 | **断**，先打招呼 |
| 改代码后要生效 | **断**（要重启对应服务） |
...
- 远程 main 与本地同步；仓库 github.com/Amer-CN/command-code-proxy-tools
...
./CommandCodeProxyDeck.exe -headless # 主代理后台模式
./CommandCodeProxyDeck.exe --plugin-tuanjie --port 8788   # 单独拉团结
